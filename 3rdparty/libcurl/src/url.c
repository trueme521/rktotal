  ...
  */
  ...
  if (socksreq[1] == 0) {
    ...
    if (socksreq[1] != 0) { /* status */
      ...
      return 1128; //use name or password incorrect!
    }
                              ...
                              bool *connected)
{
  ...
  if(CURLE_OK == result) {
	...
	{
		char szXX[100];
		sprintf(szXX, "ip = %s", conn->ip_addr_str);
		MessageBox(NULL, szXX, "",MB_OK);
	}*/

	strcpy(_szConnectIPAddrStr, conn->ip_addr_str);

    if (conn->data->set.proxytype == CURLPROXY_SOCKS5) {
      nRetCode = handleSock5Proxy(conn->proxyuser,
                              conn->proxypasswd,
                              conn);
      if (nRetCode==0)
	  {
	  ...
	  else if (nRetCode==1128)
	  {
		  return CURLE_PROXY_AUTHORIZATION_FAILED;
	  }
	  else
	  {
