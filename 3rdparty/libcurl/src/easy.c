...
extern char _szConnectIPAddrStr[257];
...
CURL_EXTERN CURLcode curl_easy_getipaddrstrinfo(CURL *curl, char *szIP)
{
	struct SessionHandle *data = (struct SessionHandle *)curl;

	strcpy(szIP, _szConnectIPAddrStr);
...
CURLcode curl_easy_getinfo(CURL *curl, CURLINFO info, ...)
{
  ...
  struct SessionHandle *data = (struct SessionHandle *)curl;
/*
  if (info == CURLINFO_CONNECT_IP_ADDR)
  {
	  strcpy( _szConnectIPAddrStr);
